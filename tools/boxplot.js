...
const BoxplotChart = forwardRef(function BoxplotChart2({
  ...
  const renderCompPie = (g, lx) => {
    ...
    const slices = g.sources.map((src, si) => {
      ...
      if (pct >= 1) return /* @__PURE__ */ React.createElement("circle", { key: si, cx: lx, cy: cy2, r, fill: col, stroke: "#000", strokeWidth: "0.5" });
      const x0 = lx + Math.sin(a0) * r;
      const y0 = cy2 - Math.cos(a0) * r;
      const x1 = lx + Math.sin(a1) * r;
      const y1 = cy2 - Math.cos(a1) * r;
      const lg = pct > 0.5 ? 1 : 0;
      return /* @__PURE__ */ React.createElement("path", { key: si, d: `M${lx},${cy2}L${x0},${y0}A${r},${r},0,${lg},1,${x1},${y1}Z`, fill: col, stroke: "#000", strokeWidth: "0.5" });
    });
