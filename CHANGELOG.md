...
- **Component CSS classes (foundation + molarity pilot)** — introduces `tools/components.css`, a stylesheet that mirrors the inline-style constants from `tools/shared.js` (`sec`, `inp`, `inpN`, `lbl`, `selStyle`, `sepSelect`, `btnPrimary`, `btnPlot`, `btnDownload`, `btnSecondary`, `btnDanger`) as `dv-panel`, `dv-input`, `dv-input-num`, `dv-label`, `dv-select`, `dv-select-sep`, `dv-btn dv-btn-primary` … and adds proper `:hover`, `:focus-visible`, `:active`, and `:disabled` states that inline styles cannot express. Wired into every HTML `<head>` alongside `theme.css`. The molarity tool has been migrated as the pilot — every former `style={sec}` / `style={{ ...sec, … }}` / `style={btnPrimary}` etc. now uses `className="dv-panel"` (plus a residual override-only `style={{}}` when a per-element tweak is needed). The remaining five tools and the shared component files still use the inline-style constants and will be migrated in follow-up commits; both styles live side by side until then.
